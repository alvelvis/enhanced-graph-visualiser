...
Util.profileStart('before render');
      ...
      var persistentHighlight = null;

      var onMouseOver = function(evt) {
        if (persistentHighlight) return; // Skip highlighting if persistent highlight is active
        handleHighlight(evt);
      };

      var onMouseOut = function(evt) {
        if (persistentHighlight) return; // Skip removing highlight if persistent highlight is active
        clearHighlight();
      };

      var onClick = function(evt) {
        var target = $(evt.target);
        if (persistentHighlight && persistentHighlight.target.is(target)) {
          // If the same element is clicked again, clear the persistent highlight
          clearHighlight();
          persistentHighlight = null;
        } else if (target.closest('.span, .arcs').length) {
          // Set the new persistent highlight only if the target is a child of a span
          clearHighlight();
          persistentHighlight = { target };
          handleHighlight(evt);
        }
      };

      function handleHighlight(evt) {
        var target = $(evt.target);
        var id;

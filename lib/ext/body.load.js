...
function conlluParse(text) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (line.includes('\t')) {
            ...
            if ($('#onlyMultipleEnhanced').is(':checked') && token[8].indexOf('|') == -1) {
                token[8] = "_";
            }
            ...
            // Aproveitar e apagar basic que são iguais
            if (token[8] !== "_"){// && $('#basicDep').is(":checked") && !$('#enhancedDep').is(":checked")) {
                if (enhanceds.indexOf(`${token[6]}:${token[7]}`) === -1){
                    head = token[6]
                    label = token[7]
            ...
            if (token[8] !== "_") {
                for (let dep of enhanceds) {
                    if (dep !== `${token[6]}:${token[7]}`) {
                        deps = dep.split(/:(.*)/s);
                        head = deps[0]
                        label = deps[1]
                        tokens[0] = `# visual-style ${head} ${token[0]} ${label} color:red\n` + tokens[0];
